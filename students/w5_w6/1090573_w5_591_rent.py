...
class House591Spider():
    ...
    def get_house_detail(self, house_id):
        ...
        if token_item is not None:
            headers['X-CSRF-TOKEN'] = str(token_item.get('content'))
        headers['deviceid'] = str(s.cookies.get_dict().get('T591_TOKEN'))
        headers['token'] = str(s.cookies.get_dict().get('PHPSESSID'))
        headers['device'] = 'pc'
...
if __name__ == "__main__":
    ...
    print('價格\t格局\t樓層\t\t說明')
    for house in houses:
        #print(house['title'], end='\t')
        print(f"{house['price']}\t{house['room_str']}\t\t{house['floor_str']}\t{house['title']}")

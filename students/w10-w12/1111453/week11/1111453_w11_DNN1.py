...
model.add(tf.keras.layers.Dense(units=768, input_dim=784,  kernel_initializer='normal',activation=activation_function, name='layers_dense')) 
model.add(tf.keras.layers.Dropout(0.2, name='layers_dropout1'))
model.add(tf.keras.layers.Dense(units=64, kernel_initializer='normal',activation=activation_function, name='layers_dense_2')) 
model.add(tf.keras.layers.Dropout(0.2, name='layers_dropout2'))

model.add(tf.keras.layers.Dense(units=64, kernel_initializer='normal',activation=activation_function, name='layers_dense_3')) 
model.add(tf.keras.layers.Dropout(0.2, name='layers_dropout3'))
model.add(tf.keras.layers.Dense(units=64, kernel_initializer='normal',activation=activation_function, name='layers_dense_4')) 
# Add output layer
...
if use_mse:
    ...
    train_history = model.fit(x=x_train_2D, y=y_train_one_hot, validation_split=0.2, epochs=100, batch_size=800, 
...
# used for loss function is sparse_categorical_crossentropy    
    ...
    train_history=model.fit(x=x_train_norm, y=y_train, epochs=100,  validation_data=(x_test_norm, y_test),batch_size=100, 
                            verbose=2,callbacks=[tensorboard_callback])
    # 顯示訓練成果(分數)

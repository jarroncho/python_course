...
model = tf.keras.models.Sequential([
    # Add Input layer, 隱藏層(hidden layer) 有 64個輸出變數
    tf.keras.layers.Dense(units=64, input_dim=784,  kernel_initializer='normal',activation='relu', name='layers_dense'), 
    tf.keras.layers.Dropout(0.2, name='layers_dropout'),
    tf.keras.layers.Dense(units=64, kernel_initializer='normal',activation='relu', name='layers_dense_2'), 
    tf.keras.layers.Dropout(0.2, name='layers_dropout_1'),
    tf.keras.layers.Dense(units=64, kernel_initializer='normal',activation='relu', name='layers_dense_3'), 
    tf.keras.layers.Dropout(0.2, name='layers_dropout_2'),
    # Add output layer
    tf.keras.layers.Dense(units=10,  kernel_initializer='normal',activation='softmax', name='layers_dense_4')
])
...
if use_mse:
    train_history = model.fit(x=x_train_norm, y=y_train_one_hot, validation_split=0.2, epochs=5, batch_size=800, 

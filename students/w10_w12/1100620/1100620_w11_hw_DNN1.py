...
# tfboard lordir
log_dir = "pythonCourse/1100620/logs/fit/" + \
...
model.add(tf.keras.layers.Dense(units=768, input_dim=784,  kernel_initializer='normal',
          activation=activation_function, name='layers_dense'))
model.add(tf.keras.layers.Dropout(0.2, name='layers_dropout'))
model.add(tf.keras.layers.Dense(units=64, kernel_initializer='normal',
          activation=activation_function, name='layers_dense_2'))
model.add(tf.keras.layers.Dropout(0.2, name='layers_dropout2'))
model.add(tf.keras.layers.Dense(units=64, kernel_initializer='normal',
          activation=activation_function, name='layers_dense_3'))
model.add(tf.keras.layers.Dropout(0.2, name='layers_dropout3'))
model.add(tf.keras.layers.Dense(units=64, kernel_initializer='normal',
          activation=activation_function, name='layers_dense_4'))
# Add output layer
model.add(tf.keras.layers.Dense(units=10,  kernel_initializer='normal',
          activation='softmax', name='layers_dense_5'))
...
if use_mse:
    ...
    train_history = model.fit(x=x_train_norm, y=y_train, epochs=20,  validation_data=(x_test_norm, y_test), batch_size=100,
                              verbose=2, callbacks=[tensorboard_callback])
    # 顯示訓練成果(分數)
